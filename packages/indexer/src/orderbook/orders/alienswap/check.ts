...
import { PartialOrderComponents } from "@/orderbook/orders/alienswap/index";
...
export const offChainCheck = async (
  ...
  if (options?.checkFilledOrCancelled) {
    ...
    const cancelled = await commonHelpers.isOrderCancelled(id, "alienswap");
  ...
  const minNonce = await commonHelpers.getMinNonce("alienswap", order.params.offerer);

  if (!minNonce.eq(order.params.counter)) {
    throw new Error("cancelled");
  }

  const conduit = new Sdk.Alienswap.Exchange(config.chainId).deriveConduit(order.params.conduitKey);
...
export const offChainCheckPartial = async (
  ...
  const conduit = new Sdk.Alienswap.Exchange(config.chainId).deriveConduit(conduitKey);
