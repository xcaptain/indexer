...
import { OrderKind } from "@reservoir0x/sdk/dist/alienswap/types";
...
import { offChainCheck, offChainCheckPartial } from "@/orderbook/orders/alienswap/check";
...
export type OrderInfo =
  | {
      kind: "full";
      orderParams: Sdk.Alienswap.Types.OrderComponents;
    ...
    try {
      ...
      if (order.params.orderType > 1) {
        if (
          ![
            ...
            AddressZero,
            // Pausable zone
            Sdk.Alienswap.Addresses.PausableZone[config.chainId],
            // Cancellation zone
      ...
      logger.warn(
        ...
        `Failed to handle order (will retry). orderParams=${JSON.stringify(
          ...
          metadata
        )}, isReservoir=${isReservoir}, alienswapOrderParams=${JSON.stringify(
          openSeaOrderParams
        )}, error=${error}`
      );
    }
      ...
      limit(async () =>
        orderInfo.kind == "partial"
          ...
          : tracer.trace("handleOrder", { resource: "AlienswapSave" }, () =>
              handleOrder(
                orderInfo.orderParams as Sdk.Alienswap.Types.OrderComponents,
                orderInfo.metadata,
